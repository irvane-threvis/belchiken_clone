...
function handleAdd() {
  if (props.item.customizable) emit('customize', props.item)
  else cart.addItem(props.item)
}
</script>
...
<template>
  <article class="menu-item" :aria-label="item.name">
    ...
    <div class="item-image" aria-hidden="true">
      ...
      <div v-if="item.image" class="img-overlay" />
    </div>
...
.placeholder-emoji { font-size: 3.5rem; line-height: 1; user-select: none; }

/* ── Body ─── */
.item-body { padding: 0.9rem 1rem 0.5rem; flex: 1; }
.item-name { font-size: 0.95rem; font-weight: 800; color: var(--bk-text); margin: 0 0 0.35rem; line-height: 1.3; }
.item-desc { font-size: 0.76rem; color: var(--bk-text-muted); line-height: 1.5; margin: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
...
.item-price { font-size: 1rem; font-weight: 900; color: var(--bk-primary); white-space: nowrap; }
...
.btn-add:hover { transform: scale(1.07); box-shadow: 0 5px 16px rgba(225,29,72,0.45); }
